...
const testUserWorngCredentials = Role('http://127.0.0.1:3000', async testController => {
    await testController
        .typeText('#username', 'TestUserr')
        .typeText('#password', 'TestPasswordd')
        .click('#button-Login');
});
...
test('Login not existing user', async testController => {
    const todoHeader = Selector('#root > section > h1');
    await testController.expect(todoHeader.exists).notOk();
})
...
test('Create and Delete Todos', async testController => {
    ...
    for (let i = 1; i <= 4; i++) {
        await testController
            ...
            .typeText('#addForm > div:nth-child(1) > input[type=text]', 'Ich bin ein Test Todo ' + i)
            .click(prioritySelector)
            .click(priorityOptions.withText(`${i}`))
...
test('set new Todo Done and UnDone', async testController => {
    ...
    await testController
        .click('#root > div.main_div__3ZTL7 > div.main_buttonsDiv__8MzRo > div.button_myTrigger__2shFM')
        .typeText('#addForm > div:nth-child(1) > input[type=text]', 'Test Todo to Set done')
        .click(prioritySelector)
        .click(priorityOptions.withText(`3`))
        .click('#addForm > div.button_myTrigger__2shFM');
